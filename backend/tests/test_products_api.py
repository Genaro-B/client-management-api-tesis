...
@pytest.fixture
def image_client(db_session, tmp_path, monkeypatch):
...
def test_upload_product_image_success(image_client, db_session):
...
def test_uploaded_image_is_served_as_static(image_client, db_session):
...
def test_upload_invalid_content_type_returns_415(image_client, db_session):
...
def test_upload_mismatched_magic_bytes_returns_415(image_client, db_session):
...
def test_upload_oversized_image_returns_413(image_client, db_session):
...
def test_upload_missing_product_returns_404(image_client):
...
def test_upload_replaces_image_and_deletes_previous_file(image_client, db_session, tmp_path):
...
def test_delete_product_image(image_client, db_session, tmp_path):
    ...
    response = image_client.delete(f"/api/v1/products/{p.id}/image")
    assert response.status_code == 200
    assert response.json()["image_url"] is None
    assert not (folder / "img.jpg").exists()


def test_delete_product_image_without_image_is_idempotent(image_client, db_session):
...
def test_delete_product_image_missing_product_returns_404(image_client):
    """Quitar imagen de un producto inexistente responde 404."""
    response = image_client.delete("/api/v1/products/9999/image")
    assert response.status_code == 404


def test_soft_delete_conserva_la_imagen(image_client, db_session):
    ...
    image_client.delete(f"/api/v1/products/{p.id}")
    assert repo.get_by_id(p.id).image_url == "/static/uploads/1/img.jpg"

    response = image_client.patch(f"/api/v1/products/{p.id}/restore")
    assert response.status_code == 200
    assert response.json()["image_url"] == "/static/uploads/1/img.jpg"


def test_update_product_partial_keeps_image(image_client, db_session):
    ...
    response = image_client.patch(f"/api/v1/products/{p.id}", json={"precio": 200})
    assert response.status_code == 200
    assert response.json()["image_url"] == "/static/uploads/1/img.jpg"
